...
（作業中）これからデータベースとしてマップデータを分離。そのために、まずマップデータのテキスト出力プログラムをつくる。
...
（とりあえず導入）タイマーの導入。 移動が早すぎると操作性が悪いし、メモリにも負担をかけかねないので、  

(完了)タイマーの導入。  戦闘の各ターンのコマンド決定後の自動進行などで使う。
...
なおセーブデータの実装は されているが、保存されるのはマップ位置など一部のデータのみ。  

マップの構造体化。（作業中） 関連ある要素をまとめる。  

（完了）とりあえずマップを構造体配列に置換するプログラムを追加した。
...
特技の実装。現状はまだ、通常物理攻撃にしか対応していない。

「逃げる」コマンドなどの実装。

まだ敵の複数バージョンに対応してない。

（完了）素早さにもとづく行動順の定義と、その行動順にもとづく戦闘の自動進行の実装を完了。
